...
void setZeroes(vector<vector<int>>& matrix) {
  ...
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++)
      if (arr_i[i] || arr_j[j]) matrix[i][j] = 0;
  return;
}
...
void setZeroes2(vector<vector<int>>& matrix) {
  ...
  for (int i = 0; i < matrix.size(); i++) {
    for (int j = 0; j < matrix[0].size(); j++) {
      ...
      if (i == 0 && matrix[i][j] == 0) col = 1;
      if (j == 0 && matrix[i][j] == 0) row = 1;
    }
  }
...
int lengthOfLongestSubstring(string s) {
  ...
  for (int i = 0; i < s.length(); i++) {
    if (occur[s[i]] > start) start = occur[s[i]];
    occur[s[i]] = i;
...
vector<vector<int>> threeSum(vector<int>& nums) {
  ...
  for (int i = 1; i < nums.size() - 1; i++) {
    if (nums[i] != nums[i - 1]) twosum(ans, nums, i + 1, nums.size() - 1, -nums[i]);
  }
  return ans;
}
...
int removeDuplicates(vector<int>& nums) {
  ...
  while (curr < nums.size()) {
    if (nums[change - 1] != nums[curr]) swap(nums[change++], nums[curr]);
    curr++;
  }
  return change;
}
...
static bool com(Job a, Job b) { return a.profit > b.profit; }
...
bool columnCheck(vector<vector<char>> mat) {
  for (int i = 0; i < 9; i++) {
    ...
    for (int j = 0; j < 9; j++) {
      if (mat[i][j] != '.') {
        if (freq.count(mat[i][j])) return false;
        freq.insert(mat[i][j]);
      }
    }
  }
  ...
  for (int i = 0; i < 9; i += 3) {
    for (int j = 0; j < 9; j += 3) {
      ...
      for (int k = i; k < i + 3; k++) {
        for (int l = j; l < j + 3; l++) {
          if (mat[k][l] != '.') {
            if (freq.count(mat[k][l])) return false;
            freq.insert(mat[k][l]);
          }
        }
      }
...
bool isValidSudoku(vector<vector<char>>& mat) { return columnCheck(mat) && rowCheck(mat) && squareCheck(mat); }
...
class Solution {
 public:
  ...
  long long minimumMoney(vector<vector<int>>& transactions) {
    ...
    if (!e.empty()) ans = max(ans, run + e[0][0]);
    if (!g.empty()) ans = max(ans, run + g[0][0]);
    return ans;
  }
};
  ...
  vector<vector<int>> palindromePairs(vector<string>& words) {
    ...
    for (int i = 0; i < words.size(); i++) {
      for (int j = 0; j < words[i].size(); j++) {
        ...
        if (map.count(pre) && isPalindrome(suf) && map[pre] != i) ans.push_back({i, map[pre]});
        if (!pre.empty() && map.count(suf) && isPalindrome(pre) && map[suf] != i) ans.push_back({map[suf], i});
      }
    }
    return ans;
  }
...
class Solution {
 public:
  int firstMissingPositive(vector<int>& nums) {
    ...
    for (int i = 0; i < nums.size(); i++)
      if (abs(nums[i]) <= nums.size() && nums[nums[i] - 1] > 0) nums[nums[i] - 1] *= -1;
 ...
 public:
  bool equationsPossible(vector<string>& equations) {
    ...
    for (auto it : equations)
      if (it[1] == '=') insert(rank, parent, it[0] - 'a', it[3] - 'a');
    for (auto it : equations)
      if (it[1] == '!' && find_parent(parent, it[0] - 'a') == find_parent(parent, it[3] - 'a')) return false;
