  ...
  void dfs(vector<vector<int>>& adj, vector<bool>& visited, vector<int>& path, int node, int j, int sum, int& res, vector<int>& amount) {
    ...
    for (auto it : adj[node]) {
      if (!visited[it]) {
        ...
        if (j < path.size() - 1) amount[path[j + 1]] = a;
      }
    }
  }
...
class Solution {
 ...
 public:
  vector<string> splitMessage(string message, int limit) { return build_string(message, limit); }
};
...
class Solution {
  ...
  */
  int check(vector<int>& a, vector<int>& b, vector<int>& c) { return (b[1] - a[1]) * (c[0] - b[0]) - (c[1] - b[1]) * (b[0] - a[0]); }
 ...
 public:
  ...
  vector<vector<int>> outerTrees(vector<vector<int>>& trees) {
    ...
    for (auto it : trees) {
      while (lower.size() >= 2 && check(lower[lower.size() - 2], lower.back(), it) > 0) lower.pop_back();
      lower.push_back(it);
    }
    for (auto it : trees) {
      while (upper.size() >= 2 && check(upper[upper.size() - 2], upper.back(), it) < 0) upper.pop_back();
      upper.push_back(it);
    }
    set<vector<int>> ans;
    ...
    for (int i = 0; i < nums.size(); i++)
      for (int j = i + 1; j < nums.size(); j++)
        for (int k = j + 1; k < nums.size(); k++)
          if (nums[j] != nums[i] && nums[k] != nums[i] && nums[k] != nums[j]) ans++;
    return ans;
  }
};
 ...
 public:
  vector<vector<int>> closestNodes(TreeNode* root, vector<int>& q) {
    ...
    for (auto it : q) {
      ...
      if (l == nums.end() && l != nums.begin()) t[0] = *prev(l);
      if (u != nums.end()) t[1] = *u;
      ans.push_back(t);
    }
    return ans;
...
class Solution {
 public:
  long long minimumFuelCost(vector<vector<int>>& roads, int seats) {
    ...
    queue<vector<int>> q;
    for (int i = 0; i < n; i++)
      if (adj[i].size() == 1) q.push({i, 0, seats - 1, 1, -1});
    while (!q.empty()) {
      auto it = q.front();
      q.pop();
      for (auto i : adj[it[0]])
...
class Solution {
  ...
  long long dfs(vector<vector<int>>& adj, int node, int p, long long& ans) {
    int cnt = 1;
    for (auto it : adj[node])
      if (it != p) cnt += dfs(adj, it, node, ans);
 ...
 public:
  long long minimumFuelCost(vector<vector<int>>& roads, int seats) {
    ...
    long long ans = 0;
    for (auto it : adj[0]) dfs(adj, it, 0, ans);
    return ans;
  }
};
//  0 1
...
class solution {
  ...
  int sa(vector<int>& nums, int i, int j) {
    ...
    int mini = INT_MAX;
    for (int k = i; k < j; k++) mini = min(mini, nums[i - 1] * nums[k] * nums[j] + sa(nums, i, k) + sa(nums, k + 1, j));
    return dp[i][j] = mini;
  }
 ...
 public:
  int matrixMultiplication(vector<int>& nums, int n) {
    ...
    for (int i = n - 1; i >= 1; i--)
      for (int j = i + 1; j < n; j++) {
        int mini = INT_MAX;
        for (int k = i; k < j; k++) mini = min(mini, nums[i - 1] * nums[k] * nums[j] + dp[i][k] + dp[k + 1][j]);
        dp[i][j] = mini;
      }
    return dp[1][n - 1];
  }
...
class Solution {
  ...
  int get_dis(vector<vector<int>>& adj, int node, int& score, int parent = -1) {
    int sum = 0;
    for (auto it : adj[node])
      if (it != parent) sum += get_dis(adj, it, score, node);
    score += sum;
    return connected_nodes[node] = sum + 1;
  }
  void sa(vector<vector<int>>& adj, int node, vector<int>& res, int parent = -1) {
    if (parent != -1) res[node] = res[parent] - 2 * connected_nodes[node] + N;
    for (auto it : adj[node])
      if (it != parent) sa(adj, it, res, node);
  }
 ...
 public:
  vector<int> sumOfDistancesInTree(int n, vector<vector<int>>& edges) {
    ...
    for (int i = 0; i < n && leaf_node == -1; i++)
      if (adj[i].size() == 1) leaf_node = i;
    connected_nodes.resize(n, 0);
    vector<int> res(n, 0);
    if (leaf_node == -1) return res;
...
void concatenatedCount(string strs) {
  ...
  for (auto it : strs) {
    if (it == ' ') {
      ...
      for (int i = 0; i < it - 'a' - 1; i++) temp += arr[i];
      arr[it - 'a']++;
    }
  }
  ans += to_string(temp);
...
class Solution {
  int sa(vector<int>& arr, int n) {
    ...
    while (!q.empty()) {
      auto it = q.front();
      q.pop();
      if (it.first == n * n - 1) return it.second;
 ...
 public:
  int snakesAndLadders(vector<vector<int>>& board) {
    ...
    for (int i = 0; i < n * n; i++) arr[i] = board[i / n][i % n];
    return sa(arr, n);
  }
};
class Solution {
 ...
 public:
  int closestMeetingNode(vector<int>& edges, int node1, int node2) {
    ...
    for (int i = 0; i < edges.size(); i++) {
      if (max(dist1[i], dist2[i]) != INT_MAX && max(dist1[i], dist2[i]) < dist) ans = i, dist = max(dist1[i], dist2[i]);
    }
    return ans;
  }
};
...
class Trie {
 ...
 public:
  ...
  void insert(string& s) {
    ...
    for (auto it : s) {
      int i = it - 'a';
      if (temp->links[i] == NULL) temp->links[i] = new node();
      temp = temp->links[i];
    }
    temp->is_complete = true;
  }
  bool get_words(node* t_root, int cnt, int idx, string& s) {
    if (!t_root) return false;
    if (idx == s.size() - 1) return t_root->is_complete && cnt >= 1;
    if (t_root->is_complete && get_words(root, cnt + 1, idx, s)) return true;
  ...
  vector<string> sa(vector<string>& s) {
    vector<string> ans;
    for (auto it : s)
      if (get_words(root, 0, 0, it)) ans.push_back(it);
    return ans;
  }
};
...
class SummaryRanges {
 ...
 public:
  ...
  vector<vector<int>> getIntervals() { return {intervals.begin(), intervals.end()}; }
};
...
class Solution {
 public:
  vector<vector<int>> sortTheStudents(vector<vector<int>>& score, int n) {
    ...
    for (auto it : score) temp.push_back({it[n], it});
    sort(temp.begin(), temp.end(), greater<pair<int, vector<int>>>());
    vector<vector<int>> res;
    for (auto it : temp) res.push_back(it.second);
    return res;
  }
};
class Solution {
 ...
 public:
  int minCost(vector<int>& nums, int k) {
    ...
    for (int i = 0, sum = 0; i < nums.size(); i++) {
      freq[nums[i]]++;
      if (freq[nums[i]] >= 2) sum += freq[nums[i]] == 2 ? 2 : 1;
      front[i] = sum;
    }
...
class Solution {
  long long sA(vector<int>& nums, int idx = 0, int cnt = 0, int prev = INT_MAX, bool f = 0, int maxi = -1) {
    ...
    long long ans = 0;
    if ((f && nums[idx] > prev) || (!f && nums[idx] < prev)) ans += sA(nums, idx + 1, cnt + 1, nums[idx], !f, max(maxi, nums[idx]));
 ...
 public:
  long long countQuadruplets(vector<int>& nums) { return sA(nums); }
};
  ...
  long long countQuadruplets(vector<int>& nums) {
    ...
    for (int i = 0; i < n; i++) {
      for (int no = 1; no <= n; no++) {
        if (nums[i] < no) left[i][no]++;
        if (i > 0) left[i][no] += left[i - 1][no];
      }
    }
    for (int i = n - 1; i >= 0; i--) {
      for (int no = 1; no <= n; no++) {
        if (nums[i] > no) right[i][no]++;
        if (i < n - 1) right[i][no] += right[i + 1][no];
      }
    }
    long long ans = 0;
    for (int j = 1; j < n - 1; j++)
      for (int k = j + 1; k < n - 1; k++)
        if (nums[j] > nums[k]) ans += left[j][nums[k]] * right[k][nums[j]];
    return ans;
  }
};
...
class LFUCache {
  ...
  Node* change_position(Node* temp, int value = -1) {
    ...
    int cnt = count[k];
    delete_node(temp);
    if (lru[cnt].first->next == lru[cnt].second) lru.erase(cnt);
    if (lru.find(cnt + 1) == lru.end()) lru[cnt + 1] = get_new_node();
    auto it = lru[cnt + 1];
  ...
  Node* get_least_used(pair<Node*, Node*> dll) { return dll.second->prev; }
 ...
 public:
  ...
  void put(int key, int value) {
    if (keys.size() == cap && keys.find(key) == keys.end()) {
      ...
      delete_node(least);
      if (it.first->next == it.second) lru.erase(lru.begin());
      count.erase(least->key);
      keys.erase(least->key);
    }
    if (keys.find(key) == keys.end()) {
      count[key]++;
      if (lru.find(count[key]) == lru.end()) lru[count[key]] = get_new_node();
...
class LRUCache {
 ...
 public:
  ...
  int get(int key) {
    if (used_keys.find(key) == used_keys.end()) return -1;
...
class LFUCache {
  ...
  int reposition_node(Node* node, int val = -1) {
    ...
    delete_node(node);
    if (is_empty(cnt)) lru.erase(cnt);
    if (lru.find(cnt + 1) == lru.end()) lru[cnt + 1] = get_node();
    count[key] = cnt + 1;
 ...
 public:
  ...
  int get(int key) {
    if (cap == 0 || used_keys.find(key) == used_keys.end()) return -1;
    // cout << "get : " << key << endl;
  ...
  void put(int key, int value) {
    ...
    if (used_keys.find(key) == used_keys.end()) {
      if (lru.find(1) == lru.end()) lru[1] = get_node();
...
int get(vector<vector<int>>& adj, vector<bool>& visited, int n) {
  ...
  int cnt = 0;
  for (auto it : adj[n]) {
    if (!visited[it]) cnt += get(adj, visited, it);
  }
  return cnt;
}
...
int main() {
  ...
  for (int i = 1; i <= n; i++)
    if (!visited[i]) ans = min(ans, get(adj, visited, i));
  cout << ans;
  return 0;
}
...
class Solution {
 ...
 public:
  vector<int> maxPoints(vector<vector<int>>& grid, vector<int>& queries) {
    ...
    for (int i = 0; i < n; i++) qu.push_back({queries[i], i});
    ...
    while (!pq.empty()) {
      ...
      while (idx < n && val >= qu[idx].first) idx++;
      if (idx == n) break;
      ans[idx]++;
      for (auto it : cor) {
        int x = it.first + i, y = it.second + j;
        if (x < 0 || y < 0 || x == grid.size() || y == grid[0].size() || visited[x][y]) continue;
        visited[x][y] = 1;
    ...
    vector<int> res(n);
    for (int i = 0; i < n; i++) res[qu[i].second] = ans[i];
    return res;
  }
};
class Solution {
 public:
  long long minCost(vector<int>& basket1, vector<int>& basket2) {
    ...
    for (int it : basket1) ++map[it];
    for (int it : basket2) --map[it];
    vector<int> ps;
    int mini = INT_MAX;
    for (auto it : map) {
      mini = min(mini, it.first);
      if (it.second % 2) return -1;
      for (int i = 0; i < abs(it.second) / 2; ++i) ps.push_back(it.first);
    }
    sort(ps.begin(), ps.end());
    long long ans = 0;
    for (int i = 0; i < ps.size() / 2; ++i) ans += min(mini * 2, ps[i]);
    return ans;
  }
};
...
class Solution {
 public:
  long long distinctNames(vector<string>& ideas) {
    ...
    for (auto it : ideas) char_suffixes_group[it[0]].insert(it.substr(1));
    for (char i = 'a'; i <= 'z'; i++) {
      for (char j = i + 1; j <= 'z'; j++) {
        int n = 0;
        for (auto it : char_suffixes_group[j])
          if (char_suffixes_group[i].find(it) != char_suffixes_group[i].end()) n++;
...
class Solution {
  ...
  vector<int> topoSort(vector<vector<int>>& adj, int n) {
    ...
    vector<int> ans;
    for (auto it : adj)
      for (int i : it) inDegree[i]++;
    for (int i = 1; i <= n; i++)
      if (inDegree[i] == 0) topoSort_util(adj, i, inDegree, ans);
    return ans;
  }
  ...
  void maxbeauty(vector<vector<int>>& adj, int node, int prev, string& s, vector<int>& freq, vector<bool>& visited, int& maxBeauty) {
    ...
    for (auto it : adj[node]) {
      if (prev != it) maxbeauty(adj, it, node, s, freq, visited, maxBeauty);
    }
    --freq[s[node] - 'a'];
  }
...
int get(vector<vector<int>>& adj, vector<bool>& visited, int n) {
  ...
  visited[n] = true;
  int cnt = 1;
  for (auto it : adj[n]) cnt += get(adj, visited, it);
  return cnt;
}
...
class Solution {
 ...
 public:
  vector<int> vowelStrings(vector<string>& words, vector<vector<int>>& queries) {
    ...
    for (int i = 1; i < words.size(); i++) pre[i + 1] = pre[i] + isVowel(words[i]);
 ...
 public:
  long long countFairPairs(vector<int>& nums, int lower, int upper) {
    ...
    for (int k = 0; k < nums.size(); k++) {
      while (0 <= i && nums[k] + nums[i] >= lower) --i;
      while (0 <= j && nums[k] + nums[j] > upper) --j;
      ans += j - i;
      if (k > i && k <= j) --ans;
    }
    return ans;
...
class Solution {
 public:
  vector<vector<int>> substringXorQueries(string s, vector<vector<int>>& queries) {
    ...
    for (int i = 0; i < s.size(); i++) {
      if (s[i] == '0') {
        if (map.find(0) == map.end()) map[0] = {i, i};
        continue;
      }
      int num = 0;
      for (int j = 0; j < 31 && j + i < s.size(); j++) {
        num = (num << 1) | (s[i + j] - '0');
        if (map.find(num) == map.end()) map[num] = {i, i + j};
      }
    }
    vector<vector<int>> ans;
    for (auto it : queries) {
...
class Solution {
  ...
  int sa(vector<string>& words, int mask, int k, int n) {
    ...
    if (newMask > mask) mini = min(mini, 1 + sa(words, newMask, k, n));
 ...
 public:
  int minStickers(vector<string>& stickers, string target) {
    indices.clear();
    for (int i = 0; i < target.size(); i++) indices[target[i]].push_back(i);
    t = target;
    memset(dp, -1, sizeof dp);
...
class Solution {
  int sa(vector<int>& cookies, int mask, int preSum) {
    ...
    for (int i = 0; i < cookies.size(); i++) {
      if (!(mask & (1 << i))) {
        if (preSum != 0) ans = min(ans, preSum);
 ...
 public:
  int distributeCookies(vector<int>& cookies, int k) { return sa(cookies, 0, 0); }
};
class Solution {
  int n;
  int sa(vector<int>& cookies, vector<int>& dist, int maxi, int k, int idx, int mask) {
    if (mask == ((1 << k) - 1) && idx == n) return maxi;
...
class Solution {
  int get_score(vector<int>& a, vector<int>& b) {
    int score = 0;
    for (int i = 0; i < a.size(); i++) score += (a[i] == b[i]);
    return score;
  }
  int n;
  int sa(vector<vector<int>>& students, vector<vector<int>>& mentors, int idx = 0, int mask = 0) {
    if (idx == n && mask == ((1 << n) - 1)) return 0;
    if (idx == n) return -1000;
    int res = -1;
    for (int i = 0; i < n; i++)
      if (!(mask & (1 << i))) res = max(res, get_score(mentors[idx], students[i]) + sa(students, mentors, idx + 1, mask | (1 << i)));
    return res;
  }
...
class Solution {
  ...
  int sa(string& s, string a, string b, int idx = 0, int maskA = 0, int maskB = 0) {
    if (idx == s.size() && !(maskA & maskB) && isPalindrome(a) && isPalindrome(b)) return a.size() * b.size();
...
class Solution {
  ...
  int count_less_than(int num) {
    int count = 0;
    for (int i = 1; i <= x; i++) count += min(num / i, y);
    return count;
  }
...
class Solution {
  ...
  int count_ugly(int a, int b, int c, int num) {
    long long ab = (1LL * a * b) / __gcd(a, b), bc = (1LL * b * c) / __gcd(b, c), ac = (1LL * a * c) / __gcd(a, c),
...
class Solution {
  int score(int n, vector<int>& nums) {
    int score = 0;
    for (auto it : nums) score += (it + n - 1) / n;
    return score;
  }
...
class Solution {
  void dfs(vector<vector<int>>& adj, int node, int& mask, int nodes, int prev = -1) {
    mask = mask | (1 << (node - 1));
    for (auto it : adj[node])
      if (it != prev && (nodes & (1 << (it - 1)))) dfs(adj, it, mask, nodes, node);
  }
  bool isValid(int nodes, vector<vector<int>>& adj) {
    int mask = 0, count = 0;
    for (int i = 0; i < 15; i++)
      if (nodes & (1 << i)) {
        if (count == 0) dfs(adj, i + 1, mask, nodes);
        count++;
      }
 ...
 public:
  vector<int> countSubgraphsForEachDiameter(int n, vector<vector<int>>& edges) {
    ...
    for (int i = 1; i <= (1 << 16) - 1; i++) {
      if (isValid(i, adj)) ans[getDiameter(adj, i) - 2]++;
    }
    return ans;
  }
};
...
class Solution {
 public:
  int findMaximizedCapital(int k, int w, vector<int>& profits, vector<int>& capital) {
    ...
    for (int i = 0; i < profits.size(); i++) projects[i] = {capital[i], profits[i]};
    ...
    int i = 0;
    while (k--) {
      while (i < projects.size() && projects[i].first <= w) pq.push(projects[i++].second);
      if (!pq.empty()) {
        w += pq.top();
        pq.pop();
      }
...
class Solution {
 ...
 public:
  int numSquarefulPerms(vector<int>& nums) { return sa(nums, nums.size()); }
};
...
class Solution {
  void conquer(vector<int>& nums, int left, int mid, int right, vector<int>& temp) {
    ...
    while (m <= right) temp[idx++] = nums[m++];
    for (int i = left, idx = 0; i <= right; i++, idx++) nums[i] = temp[idx];
  }
...
class QuickSort {
 ...
 public:
  void sort_array(vector<int>& nums) { sort_util(nums, 0, nums.size() - 1); }
};
...
class Solution {
  ...
  void modifyArray(vector<char>& chars, int& i, string& s) {
    for (int j = 0; j < s.size(); j++) chars[i++] = s[j];
  }
...
class Solution {
  ...
  void sa(vector<int>& people_skill_bits, int acquired_skills, int idx, long long& ans, long long temp) {
    if (idx == people_skill_bits.size() || acquired_skills == skills) {
      if (acquired_skills == skills && (__builtin_popcount(ans) > __builtin_popcount(temp) || ans == 0)) ans = temp;
      return;
    }
    if (ans != 0 && __builtin_popcount(ans) <= __builtin_popcount(temp)) return;
 ...
 public:
  vector<int> smallestSufficientTeam(vector<string>& req_skills, vector<vector<string>>& people) {
    ...
    for (int i = 0; i < people.size(); i++) {
      int people_skill = 0;
      for (auto it : people[i]) people_skill |= (1 << skill_idx[it]);
...
class Solution {
  ...
  void sa(vector<int>& people_skill_bits, int acquired_skills, int idx, vector<int>& temp) {
    if (idx == people_skill_bits.size()) {
      if (acquired_skills == skills && (ans.size() > temp.size() || ans.empty())) ans = temp;
      // cout << acquired_skills << " ";
      return;
    }
    if (dp[idx][acquired_skills] != -1 && dp[idx][acquired_skills] <= temp.size()) return;

    sa(people_skill_bits, acquired_skills, idx + 1, temp);
    temp.push_back(idx);
    sa(people_skill_bits, acquired_skills | people_skill_bits[idx], idx + 1, temp);
    temp.pop_back();
    if (temp.size() > 0) dp[idx][acquired_skills] = temp.size();
  }
 ...
 public:
  vector<int> smallestSufficientTeam(vector<string>& req_skills, vector<vector<string>>& people) {
    ...
    skills = (1 << n) - 1;
    // cout << skills;
    for (int i = 0; i < n; i++) skill_idx[req_skills[i]] = (1 << i);
    vector<int> people_skill_bits(people.size());
    for (int i = 0; i < people.size(); i++) {
      int people_skill = 0;
      for (auto it : people[i])
        if (skill_idx.count(it)) people_skill |= skill_idx[it];
      people_skill_bits[i] = people_skill;
    }
    vector<int> temp;
...
class Solution {
 public:
  long long countSubarrays(vector<int>& nums, int minK, int maxK) {
    ...
    for (int i = 0; i < nums.size(); i++) {
      if (nums[i] < minK || nums[i] > maxK) latest_out_range = i;
...
class Solution {
 ...
 public:
  int vowelStrings(vector<string>& words, int left, int right) {
    int ans = 0;
    for (; left <= right; left++)
      if (isvowel(words[left][0]) && isvowel(words[left].back())) ans++;
    return ans;
  }
};
class Solution {
 ...
 public:
  int findMinimumTime(vector<vector<int>>& tasks) {
    ...
    for (auto it : tasks) {
      ...
      int duration = it[2];
      for (; s <= e && d; s++) d -= line[s];
      for (; e >= it[0] && d; e--)
        if (!line[e]) line[e] = true, d--;
    }
...
class Solution {
 ...
 public:
  ListNode* mergeKLists(vector<ListNode*>& lists) {
    ListNode* head = NULL;
    for (ListNode* it : lists) head = mergeTwo(it, head);
    return head;
  }
};
class Solution {
...
class Solution {
  void getSum(TreeNode* root, vector<long>& sums, int depth = 0) {
    if (!root) return;
    if (depth == sums.size()) sums.push_back(0);
    sums[depth] += root->val;
...
class Solution {
 ...
 public:
  int findValidSplit(vector<int>& nums) {
    ...
    for (auto num : nums)
      for (int it : get_p_factors(num)) right[it]++;

    for (int i = 0, common = 0; i < n - 1; i++) {
      for (auto it : get_p_factors(nums[i])) common += (left[it]++ == 0) - (left[it] == right[it]);
  ...
  int waysToReachTarget(int target, vector<vector<int>>& types) {
    ...
    for (int i = 0; i < types.size(); i++)
      for (int t = target; t > 0; --t)
        for (int j = 1; j <= types[i][0] && t - types[i][0] * j >= 0; j++) dp[t] = (dp[t] + dp[t - types[i][1] * j]) % mod;
...
class Solution {
 ...
 public:
  int countWays(vector<vector<int>>& ranges) {
    ...
    int prev = ranges[0][1];
    for (auto it : ranges) {
      if (it[0] > prev) n_independent_ranges++;
      prev = max(it[1], prev);
    }
...
class xTrie {
 ...
 public:
  xTrie() { root = new yNode(); }

  void insert(string word) {
    yNode* temp = root;
    for (auto it : word) {
      if (!temp->next[it - 'a']) temp->next[it - 'a'] = new yNode();
      temp = temp->next[it - 'a'];
    }
  ...
  bool search(string word) {
    yNode* temp = root;
    for (auto it : word) {
      if (!temp->next[it - 'a']) return false;
      temp = temp->next[it - 'a'];
    }
...
class BrowserHistory {
 ...
 public:
  BrowserHistory(string homepage) { pages[0] = homepage; }
...
class Solution {
 public:
  int distMoney(int money, int children) {
    ...
    if (money > 8 && money % 8 == 4) return children - 2;
    return money / 8;
  }
};
class Solution {
 public:
  int maximizeGreatness(vector<int>& nums) {
    ...
    for (int i = nums.size() - 1; i >= 0; i--) {
      if (!st.empty() && st.top() > nums[i]) st.pop(), ans++;
      st.push(nums[i]);
    }
    return ans;
  }
  ...
  long long findScore(vector<int>& nums) {
    ...
    while (!pq.empty()) {
      if (!marked[pq.top().second]) {
        ...
        if (pq.top().second > 0) marked[pq.top().second - 1] = 1;
        if (pq.top().second < n - 1) marked[pq.top().second + 1] = 1;
        ans += pq.top().first;
      }
      pq.pop();
    }
 ...
 public:
  long long repairCars(vector<int>& ranks, int cars) {
    ...
    auto canRepair = [&](long long time) {
      int carsRepaired = 0;
      for (auto it : ranks) carsRepaired += sqrt(time / it);
...
class WordDictionary {
 ...
 public:
  WordDictionary() { root = new xNode(); }

  void addWord(string word) {
    xNode* temp = root;
    for (auto it : word) {
      if (!temp->next[it - 'a']) temp->next[it - 'a'] = new xNode();
      temp = temp->next[it - 'a'];
    }
    temp->isComplete = true;
  }
  ...
  bool search_sa(string& s, int i, xNode* temp) {
    ...
    if (s.size() == i) return temp->isComplete;
    bool a = false;
    if (s[i] == '.')
      for (int x = 0; x <= 26; x++) a |= search_sa(s, i + 1, temp->next[x]);
    else
...
class Solution {
  ...
  int sa(vector<pair<int, int>>& nums, int idx, int k, int mask = 0) {
    ...
    if (map.find(nums[idx].first - k) == map.end()) {
      ...
      if (--map[nums[idx].first] == 0) map.erase(nums[idx].first);
    }
...
class Solution {
 public:
  int findSmallestInteger(vector<int>& nums, int value) {
    unordered_map<int, int> map;
    for (auto& it : nums) it = map[(it % value + value) % value]++;
    int ans = 0;
    while (1) {
      if (map.find(ans % value) == map.end()) return ans;
      if (--map[ans % value] == 0) map.erase(ans % value);
      ans++;
    }
    return 1;
  }
...
class Solution {
 public:
  int minNumber(vector<int>& nums1, vector<int>& nums2) {
    ...
    int ans = INT_MAX;
    for (auto it : nums2)
      if (set.count(it)) ans = min(ans, it);
...
class Solution {
 public:
  int maximumCostSubstring(string s, string chars, vector<int>& vals) {
    ...
    for (int i = 0; i < chars.size(); i++) map[chars[i]] = vals[i];
    int ans = 0, temp = 0;
    for (auto it : s) {
...
class Solution {
  int sa(vector<vector<int>>& adj, vector<int>& visited, int node, int temp = 1) {
    ...
    int res = INT_MAX;
    visited[node] = temp;
    for (auto it : adj[node]) res = min(res, sa(adj, visited, it, temp + 1));
    return res;
  }
 ...
 public:
  int findShortestCycle(int n, vector<vector<int>>& edges) {
    ...
    for (int i = 0; i < n; i++) ans = min(sa(adj, vector<int>(n) = {}, i), ans);
  ...
  vector<int> get_max(vector<vector<int>>& adj, vector<bool>& visited, int node, string& color) {
    ...
    for (auto it : adj[node])
      if (!visited[it]) {
        ...
        for (int i = 0; i < 26; i++) temp[i] = max(temp[i], x[i]);
      } else {
        for (int i = 0; i < 26; i++) temp[i] = max(temp[i], dp[it][i]);
      }
    temp[color[node] - 'a']++;
    return dp[node] = temp;
  }
...
class Solution {
  ...
  long dfs(TreeNode* root, unordered_map<int, long>& sums, int level) {
    ...
    if (root->left) root->left->val = sums[level + 1] - sum;
    if (root->right) root->right->val = sums[level + 1] - sum;
    return root->val;
  }
...
class Graph {
 ...
 public:
  ...
  int shortestPath(int node1, int node2) { return find_shortest_path(node1, node2); }
};
...
class Solution {
  void dfs(vector<vector<int>>& adj, vector<int>& prices, int x, int y, int node, int end, int& ans, int sum = 0, int prev = -1) {
    ...
    for (auto it : adj[node])
      if (it != prev) dfs(adj, prices, x, y, it, end, ans, f, node);
  }
...
class Solution {
  void dfs(vector<vector<int>>& adj, vector<int>& prices, int node, int end, unordered_map<int, int>& map, int prev = -1) {
    map[node]++;
    for (auto it : adj[node])
      if (it != prev) dfs(adj, prices, it, end, map, node);
  }
 ...
 public:
  int minimumTotalPrice(int n, vector<vector<int>>& edges, vector<int>& price, vector<vector<int>>& trips) {
    ...
    for (auto it : trips) dfs(adj, price, it[0], it[1], map);
    priority_queue<int> pq;
...
class Solution {
  ...
  int sa(vector<string>& words, string& target, int k = 0, int i = 0) {
    if (i == target.size() || i == words[0].size() || words[0].size() - k < target.size() - i) return i == target.size();
    if (dp[i][k] != -1) return dp[i][k];
    int temp = sa(words, target, k + 1, i);
    for (int j = 0; j < words.size(); j++)
      if (words[j][k] == target[i]) temp = (0LL + temp + sa(words, target, k + 1, i + 1)) % mod;
...
class Solution {
 ...
 public:
  int numWays(vector<string>& words, string target) {
    ...
    for (int i = 0; i < n; i++)
      for (int j = 0; j < words.size(); j++) freq[i][words[j][i] - 'a']++;
    return sa(words, target, freq);
  }
};

static bool comparator(const pair<int, string>& a, const pair<int, string>& b) { return a.first > b.first; }
vector<string> Sorter(vector<string> WebPage, vector<int> Backlinks) {
  vector<pair<int, string>> pages(WebPage.size());
  for (int i = 0; i < WebPage.size(); i++) pages[i] = {Backlinks[i], WebPage[i]};
  sort(pages.begin(), pages.end(), comparator);
  vector<string> ans;
  for (auto it : pages) ans.push_back(it.second);
  return ans;
}
...
long long popularity(int N, vector<long long> Popularity, vector<vector<int>> uv, int q, vector<vector<int>> abx) {
  ...
  long long ans = 0;
  for (int i = 0; i < N; i++) ans += ((i & 1) ? Popularity[i] * -1 : Popularity[i]);
  return abs(ans);
}

class Solution {
 private:
  void recurse(int current, vector<int>& present, vector<int>& future, vector<vector<int>>& adj, vector<vector<vector<int>>>& statesProfit,
               int budget) {
    ...
    for (int parentBought = 0; parentBought <= 1; parentBought++) {
      ...
      for (int b = 0; b <= budget; ++b) bestProfitAtU[b] = max(bestProfitAtU[b], childrenProfitIfNotBought[b]);
      ...
      if (price <= budget) {
        ...
        for (int b = price; b <= budget; b++) bestProfitAtU[b] = max(bestProfitAtU[b], childrenProfitIfBought[b - price] + profit);
      }
 ...
 public:
  int maxProfit(int n, vector<int>& present, vector<int>& future, vector<vector<int>>& hierarchy, int budget) {
    vector<vector<int>> adj(n, vector<int>());
    for (auto& it : hierarchy) adj[it[0] - 1].push_back(it[1] - 1);
